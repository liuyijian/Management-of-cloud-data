...
def search(title, author, publisher):
    ...
    for result_id in result_id_list:
        temp = my_table.find_one({'_id': ObjectId(result_id)}, {'_id': 0, 'title_hash': 0})
        if temp['title'] == title:
            if temp['source'] == '豆瓣网':
                temp["coverUrl"] = 'https://images.weserv.nl/?url=' + temp["coverUrl"][7:]
...
def backend():
    ...
    response = requests.post(
        'https://aip.baidubce.com/rest/2.0/ocr/v1/accurate_basic?access_token=24.3c556cd705ab5533a4d084b02839dabe.2592000.1576826055.282335-17815049',
        data=data,
    ...
    temp = itertools.permutations(words, 3)
    permutations = []
    for item in temp:
        permutations.append(list(item))
    max_length = -1
    for item in permutations:
        if request.form['title']:
            item[0] = request.form['title']
        if request.form['author']:
            item[1] = request.form['author']
        if request.form['publisher']:
            item[2] = request.form['publisher']
    temp = []
    for item in permutations:
        temp.append(tuple(item))
    permutations = list(set(temp))
    # 搜索结果并返回
    for item in permutations:
